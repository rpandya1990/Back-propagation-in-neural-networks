        ...
        for h in range(len(self.hidden_layer.neurons)):
            for i in range(self.num_inputs):
                if not hidden_layer_weights:
                    self.hidden_layer.neurons[h].weights.append(random.uniform(-1,1))
                else:
        ...
        for o in range(len(self.output_layer.neurons)):
            for h in range(len(self.hidden_layer.neurons)):
                if not output_layer_weights:
                    self.output_layer.neurons[o].weights.append(random.uniform(-1,1))
                else:
...
target_error = input("Enter Target error: ");
i = 1
while 1:
    training_inputs, training_outputs = random.choice(training_sets)
    nn.train(training_inputs, training_outputs)
    actual_error = nn.calculate_total_error(training_sets)
    print(i, actual_error)
    i = i + 1
    if actual_error<=target_error:
        print("Error is now below target error...Stopping \n")